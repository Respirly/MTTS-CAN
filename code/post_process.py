...
def calculate_HR(pxx_pred, frange_pred, fmask_pred, pxx_label=None, frange_label=None, fmask_label=None):
    pred_HR = np.take(frange_pred, np.argmax(np.take(pxx_pred, fmask_pred), 0))[0] * 60
    if pxx_label is not None:
        ground_truth_HR = np.take(frange_label, np.argmax(np.take(pxx_label, fmask_label), 0))[0] * 60
    else:
        ground_truth_HR = 0
        #ground_truth_HR = eval(pred_HR)
    return pred_HR, pred_HR #ground_truth_HR
...
def calculate_metric(predictions, labels=[], signal='pulse', window_size=360, fs=30, bpFlag=True):
    ...
    for j in range(0, data_len, window_size):
        ...
        # plt.plot(f_prd,pxx_pred.flatten())
        # plt.show()
        if signal == 'pulse':
...
if __name__ == '__main__':
    ...
    MAE, RMSE, meanSNR, HR0, HR = calculate_metric(signal, signal='pulse', fs=fs, bpFlag=True)
